...
def save_logs_to_csv(session_data):
  with open("calculator_history.csv", mode="w", newline="") as file:
     writer = csv.writer(file)
     writer.writerow(["Timestamp", "Operation Domain", "Mathematical Equation", "Calculated Yield"])
     for row in session_data:
      current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
      full_row = [current_time] + row
      writer.writerow(full_row)
  print("\n [SYSTEM SECURITY]: Active RAM session permanently compiled to 'calculator_history.csv'!")
...
while True:
  ...
  if choice == "5":
    if master_session_logs:
      save_logs_to_csv(master_session_logs)
    else:
      print("\n [SYSTEM LOG]: No active transaction recorded this session. File creation skipped.")
